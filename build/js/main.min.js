...
$(".order").validate({
    ...
    $.ajax({
      ...
      success: function success(e) {
        "ok" === e ? ($.fancybox.open({
          src: "#hidden",
          type: "inline"
        }), $(".order").find(".tags").remove(), $(".order").find(".attach__content").remove(), $(".order").trigger("reset"), $(".order .select .select__item_base .select__value").text("Любые"), $(".slider_under-price").slider("value", 0)) : alert("Произошла ошибка, проверьте корректность ввода данных.");
      }
    });
  }
}), $(window).keyup(function (e) {
  var t = $(".checkbox input:focus");
  ...
  e.preventDefault();
  var t = $(this).text();
  $(this).toggleClass("table__button-active"), $(this).text("В корзину" == t ? "В корзине" : "В корзину");
}), $("body").on("click", ".number__minus, .number__plus", function () {
  var e = $(this).closest(".number"),
      t = e.find(".number__text"),
      a = e.data("step"),
      n = parseFloat(t.val());
  return $(this).hasClass("number__minus") ? n -= a : n += a, n < 0 ? cleaveCount.setRawValue("0") : cleaveCount.setRawValue(n), !1;
}), $(function () {
  $(".slider_under-price").slider({
    ...
    slide: function slide(e, t) {
      cleavePrice.setRawValue(t.value);
    }
  ...
  var e = $(this).parent().parent().parent(),
      t = $(".select__item_base").text().trim(),
      a = $(this).find(".select__value").text();
  e.find("input").val(a), e.find(".select__item_base .select__value").text(a), $(this).find(".select__value").text(t), $(".select_open").removeClass("select_open");
  ...
  var t = [],
      a = "";
  $(this).val().length > 3 && t.push("" + $("#wishes").val()), $(".tags__value").each(function (e, a) {
    t.push("" + $(a).text());
  }), a = t.join(", "), $("#tags").val(a);
...
function sendFiles(e) {
  var t = new FormData();
  $(e).each(function (e, a) {
    a.size <= 5242880 && ("image/png" == a.type || "image/jpeg" == a.type) && (t.append("images[]", a), $(".attach").append("<div class=\"attach__content\"><div class=\"attach__content_delete\"><img src=\"./img/x-close.svg\" alt=\"x\"></div><div class=\"attach__content_name\">".concat(a.name, "</div><progress class=\"attach__content_progress\" value=\"100\" max=\"100\"></progress></div>")));
  });
}
...
$(".attach__input").focus(function () {
  ...
  var t = e.pageX - dropZone.offset().left,
      a = e.pageY - dropZone.offset().top;
  (t < 0 || t > dropZone.width() || a < 0 || a > dropZone.height()) && dropZone.removeClass("dragover");
}), dropZone.on("drop", function (e) {
  dropZone.removeClass("dragover"), sendFiles(e.originalEvent.dataTransfer.files);
